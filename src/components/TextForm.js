  ...
  return (
    <>
      <div className="mb-3 container my-3">
        ...
        <div className='my-3'>
          <button type="button" id="liveAlertBtn" className="btn btn-primary mx-1" onClick={copyText}>Copy Text</button>
          <button type="button" className="btn btn-primary mx-1" onClick={changeUpCase}>Change To Uppercase</button>
          <button type="button" className="btn btn-primary mx-1" onClick={changeLoCase}>Change To Lowercase</button>
          <button type="button" className="btn btn-primary mx-1" onClick={handleCapitalize}>Capitalize</button>
          <button type="button" className="btn btn-danger mx-1" onClick={clearText}>Clear Text</button>
          <button type="button" className="btn btn-warning mx-1" onClick={speak}>Speak</button>
        </div>
        <p><b>{text.split(' ').filter(function (n) { return n !== '' }).length}</b> words <b>{text.length}</b> characters</p>
        <p>Estimated read time: <b>{Math.ceil(text.split(' ').filter(function (n) { return n !== '' }).length / 150)} minute(s)</b></p>
        <p><b>Preview Text: </b>{text}</p>
      </div>
    </>
  )
}
